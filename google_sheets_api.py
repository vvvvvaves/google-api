...
class GoogleSheetsHandler:
    ...
    def get_columns(self, spreadsheet_id, sheet_id):
        rows = self.get_rows_from_range(spreadsheet_id, sheet_id, 1, 2)
        return rows[0]
    ...
    def add_sheet_to_spreadsheet(self, spreadsheet_id, sheet_title=None, num_columns=50, num_rows=None):
        """
        ...
        try:
            ...
            if sheet_title is None:
                http = google_auth_httplib2.AuthorizedHttp(self.creds, http=httplib2.Http())
                spreadsheet = self.service.spreadsheets().get(spreadsheetId=spreadsheet_id).execute(http=http)
    ...
    def create_table_from_schema(self, spreadsheet_id, sheet_id, schema_path, table_name="Flashcards Table", start_row=0, start_col=0):
        ...
        try:
            ...
            http = google_auth_httplib2.AuthorizedHttp(self.creds, http=httplib2.Http())
            width_response = self.service.spreadsheets().batchUpdate(
                spreadsheetId=spreadsheet_id,
            ...
            http = google_auth_httplib2.AuthorizedHttp(self.creds, http=httplib2.Http())
            wrap_response = self.service.spreadsheets().batchUpdate(
                spreadsheetId=spreadsheet_id,
    ...
    def _get_sheet_name_by_id(self, spreadsheet_id, sheet_id):
        """
    ...
    def add_rows_to_sheet(self, spreadsheet_id, sheet_id, rows, column_order):
        """
        ...
        sheet_name = self._get_sheet_name_by_id(spreadsheet_id, sheet_id)
        if not sheet_name:
    ...
    def get_rows_from_range(self, spreadsheet_id, sheet_id, start_row, end_row, start_col=None, end_col=None):
        """
    ...
    def get_all_rows_from_sheet(self, spreadsheet_id, sheet_id):
        """
...
class SheetHandler(GoogleSheetsHandler):
    ...
    def get_dataframe(self):
        ...
        self.df = df
        self.df_last_updated = datetime.now()
        return self.df
    ...
    def get_columns(self):
        ...
        if not self.sheet_id:
            ...
            return None
        if self.columns is None:
            print("Getting columns: ", self.spreadsheet_id, self.sheet_id)
            self.columns = super().get_columns(self.spreadsheet_id, self.sheet_id)
        return self.columns
        
    def create_spreadsheet(self, title):
        self.spreadsheet_id = super().create_spreadsheet(title)
        self.sheet_id = 0
    
    def add_sheet_to_spreadsheet(self, sheet_title=None):
        if not self.spreadsheet_id:
            print("Spreadsheet not created yet")
            return None
    ...
    def add_rows_to_sheet(self, rows, column_order):
        ...
        if not self.sheet_id:
            print("Sheet not added yet")
            return None
        result = super().add_rows_to_sheet(self.spreadsheet_id, self.sheet_id, rows, column_order)
        return result
...
def main():
    ...
    sheets_handler.add_rows_to_sheet(spreadsheet_id, sheet_id, example_data, example_column_order)
